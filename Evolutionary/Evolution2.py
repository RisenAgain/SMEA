...
import numpy as np
import primefac
from DifferentialEvolution.differential_evolution_functions1 import Mutation,Repair_Solution,Select_Random
import random
data=np.array
matrix=np.array
...
def generate_matingPool(H, i, x, x_K, feature, matrix, matrix_K, beta=0):   #H, i, A[i],A_K[i], neuron_weight, neuron_K, beta=0.7
    """
...
def Generate(Idata,Q, mapp, H, population,i, x, x_K, feature,flag, max_solution_length, b, a,CR=0,F=0, mutation_prob=0,eta=0):
    ...
    y1, y2, y3 = Select_Random(Q, mapp, population,i,flag, x)
    # print "y1 : ", y1
    # print "y2 : ", y2
    # print "y3 : ", y3
    y_dash = []
    ...
    for j in range(x_count):
        if CR >= uniform(0,1): # CR value specified as 0.8
            m = y1[j] - y2[j]
            y_dash.insert(j, y3[j] + F * m)  # Value of taken as F=0.9
        else:
            y_dash.insert(j, x_copy[j])                      #Otherwise
    y_dash.extend(0 for j in range(len(x_copy)-x_count))
    y_dash = np.array(y_dash)
    print "No. of clusters for current solution {0}th solution: ", x_K
