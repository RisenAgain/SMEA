...
while t<T:
    ...
    pdb.set_trace()
    neuron_to_data_mapping_gene = mapping(A_gene, A_gene_K, neuron_weight_gene, neuron_K_gene, feature_gene)
    ...
    for i in range(len(A_population)):
        ...
        MatingPool_gene, flag_gene = generate_matingPool(H, i,temp1_gene, temp1_gene_K, feature_gene, neuron_weight_gene, neuron_K_gene, len(pop_gene_data), neuron_to_data_mapping_gene, beta=0.7)
        if flag_gene == 0:
        ...
        MatingPool_cond, flag_cond = generate_matingPool(H, i, temp1_cond, temp1_cond_K, feature_cond, neuron_weight_cond, neuron_K_cond, len(pop_cond_data), neuron_to_data_mapping_cond, beta=0.8)
        if flag_cond == 0:
