...
def form_bicluster_and_cal_objectives(cluster_gene, gene_data, label_gene, cond_cluster, cond_data, label_cond, MSR_threshold):
    """
    ...
    for kk in range(len(gene_index)):
        ...
        for jj in range(len(cond_index)):
            ...
            #print "MSR, RV, BI size, Bi index of {0}th bicluster : ".format(biclusterno), MSR_avg, RV_avg, BI_size, BI_index
    ...
    for i in range(len(bicluster_MSR)):
        #Should there be "Equal to 0"?
        if bicluster_MSR[i]>0 and bicluster_MSR[i]<MSR_threshold :
        #if bicluster_MSR[i]<MSR_threshold:
